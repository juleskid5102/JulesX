...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(message.includes('invalid') ? 'Email hoặc mật khẩu không đúng.' : message)
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="min-h-screen grid lg:grid-cols-2">
      ...
      <div className="flex items-center justify-center px-6 py-12 bg-background-light">
        <div className="w-full max-w-md space-y-8">
          ...
          <div>
            <h1 className="text-3xl font-extrabold text-stone-900 font-display">Đăng Nhập</h1>
            <p className="text-stone-500 mt-2 font-display">Truy cập tài khoản để theo dõi dự án của bạn</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-2">
              ...
              <div className="text-right">
                <a href="#" className="text-sm text-primary hover:underline font-display">Quên mật khẩu?</a>
              </div>
            </div>
            <button
              type="submit"
      ...
      <div className="hidden lg:flex flex-col items-center justify-center bg-[#101122] p-12 relative overflow-hidden">
        ...
        <div className="relative z-10 text-center space-y-8 max-w-md">
          ...
          <div className="rounded-xl overflow-hidden shadow-2xl border border-white/10 mb-6">
            <img
              src="/images/projects/jules-oasis.jpg"
              alt="Jules Oasis — Dự án mẫu"
              className="w-full h-48 object-cover"
            />
          </div>
          <h2 className="text-3xl font-bold text-white font-display">
            Theo dõi dự án realtime
          </h2>
          <div className="space-y-4 text-left">
            {['Xem tiến độ dự án', 'Tải hợp đồng & tài liệu', 'Quản lý thanh toán'].map((item) => (
